...
/// Reject names that are empty or contain path-traversal characters.
fn validate_partner_name(name: &str) -> Result<(), String> {
    if name.is_empty() {
        return Err("Partner name cannot be empty".to_string());
    }
    if name.contains('/') || name.contains('\\') || name.contains("..") {
        return Err("Partner name contains invalid characters".to_string());
    }
    Ok(())
}
...
pub async fn add_partner_member(
    ...
    if cert_info.is_expired {
        return Err("Certificate has expired and cannot be imported".to_string());
    }
