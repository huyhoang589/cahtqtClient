...
pub async fn token_scan(
    ...
    {
        if state.is_operation_running.load(Ordering::SeqCst) {
    ...
    if !dll_required_path.is_empty() {
        ...
        if p.exists() {
            match crate::htqt_ffi::HtqtLib::load(&dll_required_path) {
                Ok(lib) => {
                    *state.htqt_lib.lock().map_err(|e| format!("Lock poisoned: {e}"))? = Some(lib);
    ...
    *state.last_token_scan.lock().map_err(|e| format!("Lock poisoned: {e}"))? = None;
    ...
    *state.last_token_scan.lock().map_err(|e| format!("Lock poisoned: {e}"))? = Some(result.clone());

    // Reset token_login state based on scan result (always force re-login after scan)
    {
        let mut login = state.token_login.lock().map_err(|e| format!("Lock poisoned: {e}"))?;
...
pub async fn login_token(
    ...
    // Wrap PIN in Zeroizing immediately — no unprotected copies
    let pin = Zeroizing::new(pin);
    // Clone for the blocking closure; original stays outside for AppState storage
    let pin_for_login = pin.clone();
    ...
    let (cert_cn, verified_lib_path, verified_slot) = tokio::task::spawn_blocking(move || {
        ...
        let auth_pin = Secret::new(pin_for_login.as_str().to_string());
...
pub async fn logout_token(
    ...
    state.token_login.lock().map_err(|e| format!("Lock poisoned: {e}"))?.logout();
...
pub async fn get_token_status(state: State<'_, AppState>) -> Result<TokenStatusResponse, String> {
    let dll_found = state.htqt_lib.lock().map_err(|e| format!("Lock poisoned: {e}"))?.is_some();
...
pub async fn token_export_sender_cert(
    ...
    let cert = {
        let guard = state.last_token_scan.lock().map_err(|e| format!("Lock poisoned: {e}"))?;
        let scan = guard
            .as_ref()
