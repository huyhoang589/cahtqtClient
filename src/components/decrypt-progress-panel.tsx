...
function formatLine(p: DecryptProgress): string {
  if (p.status === "success") return `${p.file_name}: success`;
  if (p.status === "processing") return `${p.file_name}: decrypting…`;
  // "error"
  if (p.error) {
...
export default function DecryptProgressPanel({ progress, result, isRunning }: Props) {
  ...
  const dedupedMap = new Map<string, DecryptProgress>();
