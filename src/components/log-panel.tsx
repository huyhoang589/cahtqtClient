...
const LEVEL_COLOR: Record<LogEntry["level"], string> = {
  info:    "var(--cahtqt-text-log-info)",
  success: "var(--cahtqt-text-log-success)",
  warning: "var(--cahtqt-text-log-warning)",
  error:   "var(--cahtqt-text-log-error)",
};
  ...
  return (
    <div
      style={{
        ...
        borderTop: "1px solid var(--cahtqt-border-dark)",
        position: "relative",
        display: "flex",
    ...
    >
      ...
      >
        {entries.length === 0 ? (
          <span style={{ color: "var(--cahtqt-text-secondary)" }}>No log entries yet.</span>
        ) : (
          entries.map((entry) => (
            <div key={entry.id} style={{ lineHeight: "var(--cahtqt-line-height-base)" }}>
              <span style={{ color: "var(--cahtqt-text-secondary)" }}>[{entry.timestamp}]</span>{" "}
              <span
                style={{
              ...
              <span style={{ color: "var(--cahtqt-text-primary)" }}>{entry.message}</span>
            </div>
          ))
        )}
      </div>
