...
const STATUS_COLORS: Record<FileStatus, string> = {
  pending: "var(--cahtqt-bg-dialog-token)",
  encrypting: "var(--cahtqt-color-primary-dark-alt)",
  decrypting: "var(--cahtqt-color-primary-dark-alt)",
  done: "var(--cahtqt-color-done)",
  warning: "var(--cahtqt-color-warning-dark)",
  error: "var(--cahtqt-color-error-alt)",
};

function FileStatusBadge({ filePath, status }: { filePath: string; status?: FileStatus }) {
  if (!status) return null;
  const color = STATUS_COLORS[status] ?? "var(--cahtqt-text-muted)";
  const label = status === "pending" ? "—"
    ...
    : "err";
  return (
    <span style={{ fontSize: "var(--cahtqt-font-size-xs)", color, fontWeight: 600 }}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button className="btn btn-primary" onClick={addFiles} style={{ height: 28, padding: "0 12px", fontSize: "var(--cahtqt-font-size-sm)" }}>
            <FilePlus size={14} /> Add
          </button>
          {files.length > 0 && (
            <button className="btn btn-ghost" onClick={() => onFilesChange([])}
              style={{ height: 28, padding: "0 10px", fontSize: "var(--cahtqt-font-size-sm)" }}>
              Clear
            </button>
          )}
        </div>
        ...
        <table>
          ...
          <tbody>
            {files.length === 0 ? (
              ...
              files.map((f) => (
                <tr key={f}>
                  <td style={{ color: "var(--cahtqt-text-on-light)" }}>{getFileName(f)}</td>
                  <td style={{ fontSize: "var(--cahtqt-font-size-xs)", color: "var(--cahtqt-text-muted)", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {f}
                  </td>
                  {fileStatuses && (
                  ...
                  <td style={{ width: 40, textAlign: "right" }}>
                    <button className="btn-icon" onClick={() => removeFile(f)} title="Remove file"
                      style={{ color: "var(--cahtqt-text-muted)", width: 24, height: 24 }}>
                      <X size={12} />
                    </button>
                  </td>
                </tr>
