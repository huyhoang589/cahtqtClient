      ...
      <Popover.Portal>
        ...
        >
          ...
          {rows.map(([label, value]) => (
            <div
              key={label}
              style={{ display: "flex", gap: 8, marginBottom: 4, fontSize: "var(--cahtqt-font-size-sm)" }}
            >
              <span style={{ color: "var(--cahtqt-text-secondary)", minWidth: 80 }}>{label}:</span>
              <span
                style={{
                  fontFamily: label === "Serial" ? "var(--cahtqt-font-mono)" : undefined,
                  wordBreak: "break-all",
                }}
              >
                {value}
