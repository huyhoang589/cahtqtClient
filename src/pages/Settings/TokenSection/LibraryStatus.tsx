import type { LibraryInfo } from "../../../types";

interface Props {
  libraryInfo: LibraryInfo | null;
}

export default function LibraryStatus({ libraryInfo }: Props) {
  if (!libraryInfo) {
    return (
      <div style={{ color: "var(--cahtqt-text-muted)", fontSize: "var(--cahtqt-font-size-sm)" }}>
        No PKCS#11 middleware detected
      </div>
    );
  }
  return (
    <div
      style={{
        fontSize: "var(--cahtqt-font-size-sm)",
        display: "flex",
        flexDirection: "column",
        gap: 4,
      }}
    >
      <div>
        <span style={{ color: "var(--cahtqt-color-success)" }}>●</span>
        <span style={{ marginLeft: 6, color: "var(--cahtqt-text-on-light)" }}>
          {libraryInfo.vendor} — {libraryInfo.description || "PKCS#11 Library"}
        </span>
      </div>
      <div
        style={{
          fontFamily: "var(--cahtqt-font-mono)",
          fontSize: "var(--cahtqt-font-size-xs)",
          color: "var(--cahtqt-text-muted)",
        }}
      >
        {libraryInfo.path}
      </div>
      <div style={{ fontSize: "var(--cahtqt-font-size-xs)", color: "var(--cahtqt-text-muted)" }}>
        v{libraryInfo.library_version} · PKCS#11 {libraryInfo.cryptoki_version}
      </div>
    </div>
  );
}
