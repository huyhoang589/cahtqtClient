...
export default function EncryptPage() {
  ...
  const { tokenStatus: { dll_found, status: tokenStatus }, settings: { outputDataDir } } = useAppContext();
  ...
  useEffect(() => {
    ...
    (async () => {
      const unlisten = await listen("communication-cert-changed", () => {
        getCommunicationCert().then(setCommCert).catch(() => {});
      });
      if (cancelled) {
        unlisten();
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        borderBottom: `2px solid ${commCert ? "var(--cahtqt-color-success-border)" : "var(--cahtqt-color-warning-muted)"}`,
        color: commCert ? "var(--cahtqt-color-success-dark)" : "var(--cahtqt-color-warning-text)",
        display: "flex",
        alignItems: "center",
        gap: 8,
        flexShrink: 0,
      ...
      {/* Header */}
      <div style={{ padding: "16px 20px 12px", borderBottom: "1px solid var(--cahtqt-border-light)", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <h2 style={{ fontSize: "var(--cahtqt-font-size-xl)", color: "var(--cahtqt-text-on-light)" }}>Encrypt Files</h2>
